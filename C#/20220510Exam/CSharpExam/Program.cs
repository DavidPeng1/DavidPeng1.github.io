...
namespace CSharpExam
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            #region 第2題
            // 2. 列出所有在"L107"教室上課的課程ID -- 完成
            Console.WriteLine("2. 列出所有在'L107'教室上課的課程ID");
            {
                //作答區
                Console.WriteLine(
                    String.Join("\n", courseList
                    .Where(x => x.Classroom == "L107")
                    .Select(x => x.CourseId)                          
                        )          
                    );
            }
            ...
            Console.WriteLine("3. 列出所有在'L107'教室上課，並且學分為3的課程ID");
            {
                ...
                Console.WriteLine(
                    String.Join("\n", courseList
                    .Where(x => x.Classroom == "L107" && x.Credit == 3)
                    .Select(x => x.CourseId)
                        )
                    );
            }
            ...
            Console.WriteLine("6. 列出所有女性學生的名字");
            {
                ...
                Console.WriteLine(
                    String.Join("\n", studentList
                    .Where(x => x.Gender == GenderOption.Female)
                    .Select(x => x.Name)
                    )
                    );
            }
            ...
            #region 第7題
            // 7. 列出有上'Git'這門課的學員名字 -- 未完成
            Console.WriteLine("7. 列出有上'Git'這門課的學員名字");
            {
                //作答區

                var GIT = courseList.Where(x => x.Name == "Git").Select(x => x.CourseId);
                foreach(var tmpGIT in GIT)
                {
                    Console.WriteLine(
                         String.Join("\n", studentList
                            .Where(x => x.CourseList.Contains(tmpGIT))
                            .Select(x => x.Name)
                            )
                        );
                } 
            ...
            */
            ...
            foreach (var student in newlist)
            {
                ...
                Console.WriteLine(
                    String.Join(
                    "\n\t", courseList
                    .Where(x => student.CourseList.Contains(x.CourseId))
                    .Select(x => x.Name)
                        )
                    );
            }
            ...
            {
                //作答區
                var stmin = studentList.Select(x => x.CourseList.Count()).Min();
             
                Console.WriteLine(
                    String.Join("\n",studentList
                    .Where(x=>x.CourseList.Count()== stmin)
                    .Select(x => x.Name)
                    ));
            }
            ...
            Console.WriteLine("10. 找出誰修的學分總和小於10");
            {
                ...
                {
                    int total = 0;
                    foreach (var id in student.CourseList)
                    {
                        var credit = courseList
        ...
        // 多載可以傳入 IComparer<TKey> 作為自訂排序的標準
        public class ZCompare : IComparer<string>
        {
            // 建立一個依照順序的 List<string>，就可以靠該生肖在 List 中的 index 決定大小
            private static List<string> zodiacs = new List<string>()
            {
                "鼠", "牛", "虎", "兔", "龍" , "蛇", "馬", "羊" , "猴", "雞", "狗", "豬"
             };

            //  這個 ZCompare Class 不會只被用到一次
            //  所以用 Compare 方法回傳值的方式
            public int Compare(string x, string y)
            {
                //  生肖位於列表中的索引值(index)相減正好符合需求
                return (zodiacs.IndexOf(y) - zodiacs.IndexOf(x));
            }
        }
    }
}
